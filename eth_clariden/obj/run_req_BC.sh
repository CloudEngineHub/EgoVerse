#!/bin/bash
#SBATCH --job-name=T_obj_BC
#SBATCH --account=a144
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_slurm_out_v2/50hz/BC/obj/slurm-obj-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_slurm_out_v2/50hz/BC/obj/slurm-obj-%j.err
#SBATCH --nodes=1
...
export RLDB_LOAD_WORKERS=16                # Parallel dataset loading threads (default 10)
...
export hydra_run_dir=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_v2/50hz/${debug_folder}BC/${EXPERIMENT}/${task}/${task}_${frame_type}
##################################################################


export config_name=train_eth_${arm}_BC
...
export WANDB_RUN_ID=${task}_${frame_type}_${description}_BC_${PG_CL_EXPERIMENT}_${arm}_${job_id_for_wandb}

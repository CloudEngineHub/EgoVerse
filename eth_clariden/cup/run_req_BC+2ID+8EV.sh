#!/bin/bash
#SBATCH --job-name=T_cup_BC+2ID+8EV
#SBATCH --account=a144
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_slurm_out_v2/50hz/BC+2ID+8EV/cup/slurm-cup-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_slurm_out_v2/50hz/BC+2ID+8EV/cup/slurm-cup-%j.err
#SBATCH --nodes=1
...
export hydra_run_dir=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_v2/50hz/${debug_folder}BC+2ID+8EV${RESTART}/${EXPERIMENT}/${task}/${task}_${frame_type}
##################################################################
...
python /capstor/store/cscs/swissai/a144/jiaqchen/egoverse/EgoVerse/egomimic/trainHydra.py \
    ...
    trainer=${trainer} \
    logger=${logger} \
    data=cup/multi_data_BC+2ID+8EV \

#!/bin/bash
#SBATCH --job-name=T_cup_BC+2ID
#SBATCH --account=a144
#SBATCH --output=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_slurm_out_v2/50hz/BC+2ID/cup/slurm-cup-%j.out
#SBATCH --error=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_slurm_out_v2/50hz/BC+2ID/cup/slurm-cup-%j.err
#SBATCH --nodes=1
...
export RLDB_LOAD_WORKERS=32                # Parallel dataset loading threads
...
export hydra_run_dir=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_v2/50hz/${debug_folder}BC+2ID${RESTART}/${EXPERIMENT}/${task}/${task}_${frame_type}
##################################################################
...
export WANDB_RUN_ID=${task}_${frame_type}_${description}_BC+2ID_${PG_CL_EXPERIMENT}_${arm}_${job_id_for_wandb}

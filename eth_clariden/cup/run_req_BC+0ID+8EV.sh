...
export hydra_run_dir=/iopsstor/scratch/cscs/jiaqchen/egomim_out/multi_node_v2/50hz/${debug_folder}BC+0ID+8EV${RESTART}/${EXPERIMENT}/${task}/${task}_${frame_type}
##################################################################
...
export WANDB_RUN_ID=${task}_${frame_type}_${description}_BC+0ID+8EV_${PG_CL_EXPERIMENT}_${arm}_${job_id_for_wandb}
...
python /capstor/store/cscs/swissai/a144/jiaqchen/egoverse/EgoVerse/egomimic/trainHydra.py \
    ...
    trainer=${trainer} \
    logger=${logger} \
    data=cup/multi_data_BC+0ID+8EV \

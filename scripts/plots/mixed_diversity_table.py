#!/usr/bin/env python3
"""
Extract wandb validation metrics from mixed diversity experiment directories
and organize them into tables with scenes as rows and operators as columns.
"""
...
def parse_directory_name(dirname: str) -> Optional[Tuple[int, int]]:
    """
...
def main():
    ...
    # Structure: {(scenes, operators): {metric_name: value}}
    metrics_by_config: Dict[Tuple[int, int], Dict[str, float]] = {}
    
    # Scan all mixed-diversity directories
    ...
    for item in source_dir.iterdir():
        ...
        scenes, operators = parsed
        print(f"Processing: {item.name} -> {scenes} scenes, {operators} operators")
    ...
    for config in sorted(metrics_by_config.keys()):
        scenes, operators = config
        num_metrics = len(metrics_by_config[config])
        print(f"  {scenes} scenes, {operators} operators: {num_metrics} metrics")
    ...
    for scenes in expected_scenes:
        for operators in expected_operators:
            ...
            if config not in metrics_by_config:
                missing_configs.append(f"{scenes} scenes, {operators} operators")
    ...
    for metric_name in sorted(all_metrics):
        ...
        # Create DataFrame with scenes as rows and operators as columns
        data = {}
        missing_values = []
        for operators in expected_operators:
            data[operators] = []
            for scenes in expected_scenes:
                ...
                data[operators].append(value)
                if value is None:
                    missing_values.append(f"({scenes}s, {operators}o)")
        
        df = pd.DataFrame(data, index=expected_scenes)
        df.index.name = "Scenes"
        df.columns.name = "Operators"
        
        if missing_values:
            print(f"  Warning: Missing values for {metric_name} at: {', '.join(missing_values)}")

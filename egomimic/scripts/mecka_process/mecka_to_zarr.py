...
def download_with_retry(url: str, dest_path: Union[Path, str], max_retries: int = 5) -> None:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            result = subprocess.run(
                ['curl', '-L', '-C', '-', '--retry', '3',
                    '--retry-delay', '2', '-o', str(dest_path), url],
                check=True,
                capture_output=True,
                timeout=600,
                text=True
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    f"Download failed, retrying in 3 seconds... ({e})")
                if Path(dest_path).exists():
...
def pose_to_transform(pose: np.ndarray) -> np.ndarray:
    """
    Convert 6DOF pose to a 4x4 homogeneous transform matrix.

    Args:
        pose: Array of shape (6,) with [x, y, z, yaw, pitch, roll].
              Euler angles use ZYX (intrinsic) convention in radians.
...
def transform_to_pose(T: np.ndarray) -> np.ndarray:
    """
    Convert 4x4 homogeneous transform matrix to 6DOF pose.

    Args:
        T: Array of shape (4, 4) representing SE(3) transform.

    Returns:
        Array of shape (6,) with [x, y, z, yaw, pitch, roll] in radians (ZYX convention).
    """
    x, y, z = T[:3, 3]
...
def compute_hand_pose_6dof(keypoints: np.ndarray) -> np.ndarray:
    """
    Compute 6DOF pose (x, y, z, yaw, pitch, roll) from 21 hand keypoints in camera frame.
    ...
    Returns:
        Array of shape (6,) with [x, y, z, yaw, pitch, roll] in camera frame.
        Returns zeros if keypoints are all zero or if the frame cannot be computed.
    """
    ...
    if np.linalg.norm(forward) < 1e-6:
        logger.warning("Forward direction norm too small, returning position only")
    ...
    if np.linalg.norm(up) < 1e-6:
        logger.warning("Up direction norm too small, returning position only")
        return np.concatenate([position, np.zeros(3)])
    ...
    try:
        ...
        logger.warning("Rotation conversion failed, using zero orientation")
...
class MeckaExtractor:
    ...
    def process_episode(
        ...
        Returns:
            Tuple of (episode_feats, annotations_df, episode_meta):
        ...
        try:
            ...
            if local_data_dir is None:
                ...
                root_video = Path(episode_json_path).parent / \
                    f"{episode_id}_video.mp4"
                ...
                if root_video.exists():
                    ...
                    download_with_retry(
                        episode_meta["urls"]["video"], video_path)

                download_with_retry(episode_meta["urls"]["hands"], hands_path)
                download_with_retry(
                    episode_meta["urls"]["egomotion"], egomotion_path)
                download_with_retry(
                    episode_meta["urls"]["frames"], frames_path)
                download_with_retry(
                ...
                video_path = next(
                    (p for p in candidate_videos if p.exists()), None)
            ...
            for img in images:
                ...
                ds = cv2.resize(img, (target_w, target_h),
            ...
            right_keypoints = hand_keypoints_world[:, 0, :, :].reshape(num_frames, 63)
            left_keypoints = hand_keypoints_world[:, 1, :, :].reshape(num_frames, 63)

            episode_feats = {
                "right.obs_ee_pose": hand_poses_world[:, :6],
                "left.obs_ee_pose": hand_poses_world[:, 6:],
                ...
                # Placeholder wrist poses (not provided by Mecka hand data)
                "right.obs_wrist_pose": np.zeros((num_frames, 6)),
                "left.obs_wrist_pose": np.zeros((num_frames, 6)),
            }

            logger.info(f"Extracted {num_frames} frames")
            return episode_feats, annotations_df, episode_meta
    ...
    def _extract_camera_transforms(egomotion: np.ndarray) -> List[np.ndarray]:
        ...
        Translation (X,Y,Z) and quaternion (x,y,z,w) define world-to-camera (wTc).
    ...
    def _extract_hand_data(
        ...
        Hands CSV has columns: frame, hand_index (0=right, 1=left), landmark_index,
        world_x, world_y, world_z. Hand poses are computed from keypoints in camera
        frame via compute_hand_pose_6dof, then transformed to world using wTc.
        ...
        Returns:
            Tuple of (hand_poses_world, hand_keypoints_world):
                - hand_poses_world: (T, 12) [right_6dof, left_6dof] in world frame.
                - hand_keypoints_world: (T, 2, 21, 3) [right_21kp, left_21kp] in world.
        """
        ...
        for frame_idx in range(num_frames):
            for hand_index in [0, 1]:
                ...
                if len(hand_data) == 21:
                    ...
                    T_hand_cam = pose_to_transform(pose_6dof)
                    T_hand_world = wTc @ T_hand_cam
                    pose_6dof = transform_to_pose(T_hand_world)
    ...
    def _extract_head_poses(egomotion: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Array of shape (T, 7) with [x, y, z, qx, qy, qz, qw] per frame.
        """
        ...
        for i in range(len(egomotion)):
            ...
            # Output head pose as [x, y, z, qx, qy, qz, qw]
...
class MeckaDatasetConverter:
    ...
    def __init__(
        ...
        self.local_data_dir = Path(
            local_data_dir) if local_data_dir is not None else None
        ...
        self.episode_feats, self.annotations_df, self.episode_meta = MeckaExtractor.process_episode(
            episode_json_path,
            arm=arm,
    ...
    def extract_episode(self) -> None:
        ...
        ZarrWriter.create_and_write(
            ...
            # metadata_override=mecka_metadata,
        )
    ...
    def save_preview_mp4(
        ...
        resized = np.array(
            [cv2.resize(f, (outW, outH), interpolation=cv2.INTER_AREA) for f in image_frames]
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Convert Mecka RL2 to LeRobot format")
    parser.add_argument("--episode-json", required=True,
                        help="Path to episode JSON file")
    parser.add_argument("--output-dir", required=True,
                        help="Output directory for dataset")
    parser.add_argument("--repo-id", default="mecka/demo",
                        help="Dataset repo ID")
    parser.add_argument("--arm", default="both",
                        choices=["left", "right", "both"], help="Which arm(s) to include")
    parser.add_argument("--video-encoding", action="store_true",
                        help="Encode images as video in zarr")
    ...
    if output_path.exists():
        logger.warning(
            f"Output directory {args.output_dir} exists, removing...")

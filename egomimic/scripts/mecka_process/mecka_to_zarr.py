...
def compute_hand_pose_xyzquat(keypoints: np.ndarray, hand_index: int) -> np.ndarray:
   ...
   if np.linalg.norm(forward) < 1e-6:
       logger.warning("Forward direction norm too small, returning position + identity quat")
   ...
   if np.linalg.norm(up) < 1e-6:
       logger.warning("Up direction norm too small, returning position + identity quat")
       fallback = np.concatenate([position, quat_wxyz_identity])
       return fallback, fallback.copy()
   ...
   quat_wxyz = np.array([quat_xyzw[3], quat_xyzw[0], quat_xyzw[1], quat_xyzw[2]])
   return np.concatenate([position, quat_wxyz]), np.concatenate([wrist, quat_wxyz]),
       ...
       try:
           ...
           if local_data_dir is None:
               ...
               if "intrinsics" in episode_meta.get("urls", {}):
                   try:
                       download_with_retry(episode_meta["urls"]["intrinsics"], intrinsics_path)
                   except Exception as e:
           ...
           hand_poses_world, hand_keypoints_world, wrist_poses_world = MeckaExtractor._extract_hand_data(
   ...
   def _extract_hand_data(
       ...
       for frame_idx in range(num_frames):
           for hand_index in [0, 1]:
               ...
               if len(hand_data) == 21:
                   kp = hand_data[['world_x', 'world_y', 'world_z']].values  # (21, 3) in camera frame
                   ...
                   pose_xyzquat, wrist_xyzquat = compute_hand_pose_xyzquat(kp, hand_index)
                   ...
                   hand_poses[frame_idx, hand_index * 7 : (hand_index + 1) * 7] = pose_xyzquat
                   wrist_poses[frame_idx, hand_index * 7 : (hand_index + 1) * 7] = wrist_xyzquat
...
class MeckaDatasetConverter:
   ...
   def __init__(
       ...
       self.episode_feats, self.annotations_df, self.episode_meta, self.mecka_metadata = MeckaExtractor.process_episode(
           episode_json_path,
           arm=arm,

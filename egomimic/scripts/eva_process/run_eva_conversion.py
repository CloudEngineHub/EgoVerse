        ...
        with contextlib.redirect_stdout(tee_out), contextlib.redirect_stderr(tee_err):
            ...
            try:
                ...
                if backend == "zarr":
                    ...
                    zarr_candidates = sorted(ds_path.glob("*.zarr"))
                    for zp in zarr_candidates:
                        try:
                            store = _zarr.open(str(zp), mode="r")
                            frames = int(store.attrs.get("total_frames", -1))
                            zarr_store_path = zp
...
@ray.remote(num_cpus=8, resources={"eva_small": 1})
def convert_one_bundle_small(*args, **kwargs):
    return convert_one_bundle_impl(*args, **kwargs)


@ray.remote(num_cpus=32, resources={"eva_big": 1})

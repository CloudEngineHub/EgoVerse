...
class EvaHD5Extractor:
    ...
    def process_episode(
        ...
        task_description: str = "", object_description: str = "", success: bool = True
    ):
        """
        ...
        with h5py.File(episode_path, "r") as episode:
            ...
            # Add action horizon metadata
            episode_feats["metadata.action_horizon"] = np.full(
                (num_timesteps, 1), ACTION_HORIZON, dtype=np.int32
            )

            # Add success metadata
            episode_feats["metadata.success"] = np.full(
                (num_timesteps, 1), int(success), dtype=np.int32
            )

            # Note: task_description and object_description are NOT stored per-frame
            # They will be saved separately in meta/episode_metadata.json by DatasetConverter
            observation_keys = list(episode_feats["observations"].keys())

            # Add future observations and actions
            # Build future frames index map
            for t in range(num_timesteps):
                future_t = min(t + ACTION_HORIZON, num_timesteps - 1)
                
                # Add future observations
                for obs_key in observation_keys:
                    obs_data = episode_feats["observations"][obs_key]
                    future_key = f"future.{obs_key}"
                    if future_key not in episode_feats["observations"]:
                        # Initialize the future observation array with same shape as original
                        if obs_data.ndim == 1:
                            episode_feats["observations"][future_key] = np.zeros(obs_data.shape, dtype=obs_data.dtype)
                        else:
                            episode_feats["observations"][future_key] = np.zeros((num_timesteps,) + obs_data.shape[1:], dtype=obs_data.dtype)
                    
                    # Assign future value
                    if obs_data.ndim == 1:
                        episode_feats["observations"][future_key][t] = obs_data[future_t]
                    else:
                        episode_feats["observations"][future_key][t] = obs_data[future_t]
                ...
                for action_key in ["actions_joints", "actions_cartesian", "actions_base_cartesian", "actions_eef_cartesian"]:
                    if action_key in episode_feats:
                        action_data = episode_feats[action_key]
                        future_key = f"future.{action_key}"
                        if future_key not in episode_feats:
                            # Initialize the future action array with same shape as original
                            episode_feats[future_key] = np.zeros((num_timesteps,) + action_data.shape[1:], dtype=action_data.dtype)
                        
                        # Assign future value
                        episode_feats[future_key][t] = action_data[future_t]
    ...
    def extract_episode_frames(
        ...
        episode_feats = EvaHD5Extractor.process_episode(
            ...
            task_description=task_description, object_description=object_description,
            success=success
        )
                ...
                if value is not None:
                    if isinstance(value, np.ndarray):
                        if "images" in feature_id and image_compressed:
                            ...
                            # Handle string metadata - keep as is
        ...
        for key, value in episode_feats.items():
            if isinstance(value, dict):  # Handle nested dictionaries recursively
                ...
                if "images" in key:
                    ...
                    dim_names = []
                    dtype = "string"
                elif ("actions" in key or "future.actions" in key) and len(value[0].shape) > 1:
                    shape = value[0].shape
...
class DatasetConverter:
    ...
    def __init__(
        ...
        # Track episode-level metadata (task_description, object_description, success)
    ...
    def extract_episode(self, episode_path, task_description: str = "", object_description: str = "", success: bool = True, mp4_name: str = ""):
        extrinsics = EXTRINSICS[self.extrinsics_key]
        ...
        if self._mp4_path is not None:
            ...
            mp4_path = self._mp4_path / mp4_name if mp4_name else self._mp4_path / f"{ep_stem}.mp4"
        ...
        # Store episode metadata (avoid per-frame duplication)
        episode_index = self.dataset.meta.total_episodes
        self.episode_metadata[episode_index] = {
            "task_description": task_description,
            "object_description": object_description,
            "success": success,
        }
    ...
    def extract_episodes(self, episode_description: str = "", object_description: str = "", success: bool = True, mp4_name: str = ""):
        """
        ...
        object_description : str, optional
            A description of the object being manipulated (default is '').
        success : bool, optional
            Whether the episodes are successful demonstrations (default is True).
        Raises
        ------
        Exception
        ...
        for episode_path in self.episode_list:
            try:
                self.extract_episode(episode_path, task_description=episode_description, 
                                    object_description=object_description, success=success, mp4_name=mp4_name)
            except Exception as e:
        ...
        # Save episode metadata to separate JSON file
    ...
    def _save_episode_metadata(self):
        """
        Enhance episodes.jsonl with additional metadata (object_description, success).
        
        LeRobot creates episodes.jsonl with format:
            {"episode_index": 0, "tasks": ["task_desc"], "length": 300}
        
        We extend this to:
            {"episode_index": 0, "tasks": ["task_desc"], "length": 300, 
             "object_description": "...", "success": true}
        """
        ...
        episodes_path = self.dataset.root / "meta" / "episodes.jsonl"
        if not episodes_path.exists():
            self.logger.warning(f"episodes.jsonl not found at {episodes_path}")
            return
        ...
        for ep in episodes:
            ...
            if episode_index in self.episode_metadata:
                ep["object_description"] = self.episode_metadata[episode_index]["object_description"]
                ep["success"] = self.episode_metadata[episode_index]["success"]
            enhanced_episodes.append(ep)
        ...
        self.logger.info(f"Enhanced episodes.jsonl with object_description and success metadata")
...
def argument_parse():
    ...
    parser.add_argument("--object-description", type=str, default="", help="Description of the object being manipulated.")
    parser.add_argument("--success", type=str2bool, default=True, help="Set to True if episodes are successful demonstrations.")
...
RAW_ROOT = Path("/coc/flash7/scratch/egowm/raw")
PROCESSED_ROOT = Path("/coc/flash7/scratch/egowm/wmLerobotDataset")
RAW_ROOT = Path("/coc/flash7/mlin365/EgoVerse/logs/egowm_dataset/raw")
PROCESSED_ROOT = Path("/coc/flash7/mlin365/EgoVerse/logs/egowm_dataset/processed")
PROCESSED_LOCAL_ROOT = Path(os.environ.get("PROCESSED_LOCAL_ROOT", PROCESSED_ROOT)).resolve()
...
def _normalize_object_description(folder_name: Optional[str]) -> str:
    if not folder_name:
        return ""

    parts = folder_name.split("_")
    if "cup" in parts:
        cup_idx = parts.index("cup")
        first_obj = " ".join(parts[: cup_idx + 1]).strip()
        second_obj = " ".join(parts[cup_idx + 1 :]).strip()
        if first_obj and second_obj:
            return f"{first_obj}, {second_obj}"
        return first_obj or second_obj

    return folder_name.replace("_", " ").strip()


def iter_hdf5_bundles(root: Path) -> Iterator[Tuple[Path, str, Optional[str], Optional[bool]]]:
    search_paths = [root]
    demos_path = root / "demos"
    if demos_path.exists() and demos_path.is_dir():
        search_paths.append(demos_path)

    for search_root in search_paths:
        for data in sorted(search_root.glob("*.hdf5")):
            name = data.stem
            meta_json = search_root / f"{name}_metadata.json"
            extrinsics_key = _load_extrinsics_key_from_json(meta_json)
            yield data, extrinsics_key, None, None

        for obj_dir in sorted(search_root.iterdir()):
            if not obj_dir.is_dir() or obj_dir.name == "broke":
                continue

            object_folder = obj_dir.name

            for status_dir in [obj_dir / "success_demo", obj_dir / "failure_demo"]:
                if not status_dir.exists() or not status_dir.is_dir():
                    continue

                success = status_dir.name == "success_demo"

                for data in sorted(status_dir.glob("*.hdf5")):
                    name = data.stem
                    meta_json = status_dir / f"{name}_metadata.json"
                    extrinsics_key = _load_extrinsics_key_from_json(meta_json)
                    yield data, extrinsics_key, object_folder, success
...
def convert_one_bundle(
    ...
    try:
        source_h5 = Path(data_h5).resolve(strict=True)
        if not ensure_path_ready(source_h5):
            print(f"[ERR] missing {source_h5}", flush=True)
            return "", "", -1

        link = tmp_dir / source_h5.name
        try:
            os.symlink(source_h5, link, target_is_directory=False)
        ...
        print(
            f"[INFO] Converting: {data_h5} -> {ds_path} "
            f"(arm={arm}, extrinsics_key={extrinsics_key}, success={success})",
            flush=True,
        )

        _run_lerobot_conversion(
            raw_path=tmp_dir,
            output_dir=ds_parent,
        ...
        mp4_candidates = (
            list(ds_parent.glob(f"{dataset_name}_video.mp4"))
            + list(ds_path.glob(f"{dataset_name}_video.mp4"))
            + list(ds_parent.glob(f"*{dataset_name}*_video.mp4"))
            + list(ds_path.glob("**/*_video.mp4"))
        )
...
def launch(
    ...
    *,
    ...
    default_arm: str = "both",
    default_description: str = "",
    max_concurrent: int = 2,
):
    engine = create_default_engine() if use_sql else None
    pending: dict = {}
    to_process: list[dict] = []

    for data_h5, extrinsics_key, object_folder, success_status in iter_hdf5_bundles(raw_root):
        name = data_h5.stem
        object_description = _normalize_object_description(object_folder)
        success = True if success_status is None else bool(success_status)
        ...
        if use_sql:
            ...
            description = row.task_description or default_description
            item_key = episode_key
        else:
            arm = default_arm
            if object_folder and success_status is not None:
                dataset_name = f"{object_folder}_{name}_{'success' if success else 'failure'}"
            elif object_folder:
                dataset_name = f"{object_folder}_{name}"
            else:
                dataset_name = name
            description = default_description
            item_key = dataset_name

            if skip_if_done and (processed_root / dataset_name).exists():
                print(f"[SKIP] {name}: already exists at '{processed_root / dataset_name}'")
                continue
        ...
        if dry:
            ...
            mp4_candidate = processed_root / f"{dataset_name}_video.mp4"
            status_info = "" if success_status is None else f" | success={success}"
            obj_info = f" | object={object_description}" if object_description else ""
            if use_sql:
                mapped_ds = _map_processed_local_to_remote(ds_path)
                mapped_mp4 = _map_processed_local_to_remote(mp4_candidate)
                print(
                    f"[DRY] {name}: arm={arm}{status_info}{obj_info} | out_dir={out_dir}/{dataset_name}\n"
                ...
                print(
                    ...
                    f"      description={description}\n"
                    f"      extrinsics_key={extrinsics_key}\n"
                    f"      output_path={ds_path}\n"
                    f"      mp4_path={mp4_candidate.resolve()}"
                )
            continue
    ...
    if dry or not to_process:
        return

    print(f"[INFO] Processing {len(to_process)} episodes with max {max_concurrent} concurrent jobs")

    job_idx = 0
    while job_idx < len(to_process) or pending:
        while len(pending) < max_concurrent and job_idx < len(to_process):
            job = to_process[job_idx]
            print(f"[SUBMIT] Starting job {job_idx + 1}/{len(to_process)}: {job['dataset_name']}")
            ...
            ref = convert_one_bundle.remote(
                ...
                job["description"],
                job["extrinsics_key"],
                job["object_description"],
                job["success"],
            )
            pending[ref] = (job["item_key"], job["dataset_name"])
            job_idx += 1
        ...
        try:
            done_refs, _ = ray.wait(list(pending), num_returns=1, timeout=300.0)

            if not done_refs:
                print(f"[WARN] No jobs completed in 300s. {len(pending)} jobs still pending.")
                continue

            for ref in done_refs:
                ds_path, mp4_path, frames = ray.get(ref)
                item_key, _dataset_name = pending.pop(ref)

                if use_sql:
                    row = episode_hash_to_table_row(engine, item_key)
                    if row is None:
                        print(f"[WARN] Episode {item_key}: row disappeared before update?")
                        continue
                    ...
                    try:
                        ...
                        print(
                            f"[OK] Updated SQL for {item_key}: "
                            f"processed_path={row.processed_path}, num_frames={row.num_frames}"
                        )
                    except Exception as exc:
                        print(f"[ERR] SQL update failed for {item_key}: {exc}")
                else:
                    if frames is not None and int(frames) > 0:
                        print(
                            f"[OK] Converted {item_key}: "
                            f"processed_path={ds_path}, mp4_path={mp4_path}, num_frames={int(frames)}"
                        )
                    else:
                        print(f"[ERR] Conversion failed or zero frames for {item_key}")

        except Exception as exc:
            print(f"[ERR] Error in job processing loop: {exc}")
            traceback.print_exc()
            break


def wm_argument_parse():
    parser = argparse.ArgumentParser(
        description=(
            "Unified EVA->LeRobot WM conversion entrypoint. "
            "Scans raw HDF5, runs conversion, and optionally updates SQL."
        )
    )
    ...
    parser.add_argument(
        "--skip-if-done",
        action="store_true",
        help="Skip episodes that already have a processed_path in SQL (or existing folder in --no-sql mode).",
    )
    parser.add_argument(
        "--no-sql",
        action="store_true",
        help="Bypass SQL lookup/update and process files directly.",
    )
    parser.add_argument(
        "--raw-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Directory where converted datasets are written.",
    )
    parser.add_argument(
        "--default-arm",
        type=str,
        ...
        default="both",
        help="Arm to use in --no-sql mode.",
    )
    parser.add_argument(
        "--default-description",
        type=str,
        default="pick up cup with one arm, hand it over to the other arm and place it on the saucer",
        help="Task description to use in --no-sql mode.",
    )
    parser.add_argument(
        "--max-concurrent",
        type=int,
        default=2,
        help="Maximum number of concurrent conversion jobs.",
    )
    return parser.parse_args()


def wm_main() -> None:
    args = wm_argument_parse()

...
class polciy_wrapper(torch.nn.Module):
    ...
    def normalize(self, data):
        ...
        for key, value in data.items():
            ...
            if norm_key is not None:
                ...
                min = self.norm_stats[norm_key]["min"]
                max = self.norm_stats[norm_key]["max"]
                normalized[key] = 2 * ((value - min) / (max - min + 1e-6)) - 1.0
            else:
    ...
    def unnormalize(self, data):
        ...
        for key, value in data.items():
            ...
            if norm_key is not None:
                ...
                min = self.norm_stats[norm_key]["min"]
                max = self.norm_stats[norm_key]["max"]
                unnormalized[key] = (value + 1.0) * 0.5 * (max - min + 1e-6) + min
            else:
    ...
    def forward(self, data):
        ...
        B, _, _ = data['h1_ee_pose'].shape
        data['pad_mask'] = torch.ones(B, 100, 1)
